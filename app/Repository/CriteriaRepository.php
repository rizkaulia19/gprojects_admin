...
class CriteriaRepository extends BaseRepository
{
    ...
    public function findC1(string $specializationIds = ''): float
    {
        ...
        return $query->count() === 0 ? 0.0 : pow($query->count(), $this->c1_Normalize);
    }
    ...
    public function findC4(string $specializationIds = ''): float
    {
        ...
        return $query->count() === 0 ? 0.0 :  pow($query->count(), $this->c4_Normalize);
    }
    ...
    private function normalizeCriteria()
    {
        ...
        $c5 = 12.5;
        $c6 = 7.5;
        $this->c1_Normalize = - ($c1 / ($c1 + $c2 + $c3 + $c4 + $c5 + $c6));
        $this->c2_Normalize = $c2 / ($c1 + $c2 + $c3 + $c4 + $c5 + $c6);
        $this->c3_Normalize = - ($c3 / ($c1 + $c2 + $c3 + $c4 + $c5 + $c6));
        $this->c4_Normalize = $c4 / ($c1 + $c2 + $c3 + $c4 + $c5 + $c6);
        $this->c5_Normalize = $c5 / ($c1 + $c2 + $c3 + $c4 + $c5 + $c6);
        $this->c6_Normalize = - ($c6 / ($c1 + $c2 + $c3 + $c4 + $c5 + $c6));
    }
}
